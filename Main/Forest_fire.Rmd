...
I metodi usati per raggiungere gli obiettivi sono dei metodi di regressione lineare, ci permettono di spiegare una variabile risposta (Y=area bruciata) in funzione delle altre variabili esplicative a disposizione nel dataset(covariate, X). Faremo nel seguente modo:

-Step1. Useremo tutti i dati a nostra disposizione visto che non sarebbe sufficientemente esaustivo usare i dati di un campione cosi piccolo per spiegare l'intera popolazione, alleneremo 3 modelli di diversa complessità.
...
#use lasso regression model to predict response value
new = model.matrix(area ~ temp+I(temp^3)+wind+RH+I(RH^2)+I(RH*wind)+I(temp^2*wind^2)+I(RH*temp)+I(temp*ISI),data=forest_subset)
...
```{r, message = F}
media_area=mean(forest_subset$area)
best_lambda
```
