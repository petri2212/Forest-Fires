...
model5 = lm(area ~ (rain^2)+(RH^2)+(temp^2)+(FFMC^2)+(DMC^2)+(ISI^2), data = train)
summary(model5)

model6 = lm(area ~ temp+(rain*temp)+(temp*FFMC)+(RH*FFMC)+(train$day=="tue")+(train$day=="wed")+(train$day=="thu"), data = train)#non capisco perche un estimate di -82?????what does it mean bro
summary(model6)
...
library(ggplot2)
library(hrbrthemes)
p2 <- ggplot(train, aes(x=train$area, y=train$ISI)) +
  geom_point() +
...
model2 = lm(area ~rain+RH+temp+DMC, data = train)
summary(model2)
# Modello con variabili scelte da noi (scelte casualmente al momento):
model3 = lm(area ~ ., data = train)
summary(model3)

model6 = lm(area ~ temp+(rain*temp)+(temp*FFMC)+(RH*FFMC)+(day)+wind, data = train)#non capisco perche un estimate di -82?????what does it mean bro
summary(model6)
...
a = c(AIC(model6), BIC(model6),AIC(model5), BIC(model5),AIC(model3), BIC(model3), AIC(model2), BIC(model2), AIC(model1), BIC(model1))
...
b = c("AIC lm 6", "BIC lm 6", "AIC lm 5", "BIC lm 5","AIC lm 3", "BIC lm 3", "AIC lm 2", "BIC lm 2", "AIC lm 1", "BIC lm 1")
